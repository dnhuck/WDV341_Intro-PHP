...
<body>
	...
	<?php
		$validForm = false; // the form has errors
			// ternary ($validForm) ? echo "" : echo "class='errorMessage' ";
			// not operator  echo (!$validForm)? "class='errorMessage' ";
			//echo ($validForm)? "" : "class='errorMessage' ";
			//echo (!$validForm)?"class='errorMessage'";
			if(!$validForm){echo "class='errorMessage'";
	?>
		...
		<form>
			...
			<p>
				<label>Username: 
					<input type="text"> <span <?php echo "class='errorMessage'" ?>></span>
				</label>
			</p>
			<p>
				<label>Password: 
